...
for row in df_test.itertuples():
    ...
    predicted_idle_power = regr.predict([[test_processor_speed, test_cores_count, test_memory, test_processor_threads, test_power_rating, test_reference_age]])

    error_percentage = (abs(predicted_idle_power - idle_power)  / idle_power ) * 100
    total_error_percentage += error_percentage


    print (test_processor_speed, ",", test_cores_count, ",",  test_memory, ",", test_processor_threads, ",", test_power_rating, ",", test_reference_age, ",", fully_utilized_power, ",", predicted_idle_power, ",", str(error_percentage))
