...
figure5 = plt.Figure(figsize=(5, 4), dpi=100)
ax5 = figure5.add_subplot(111)
ax5.scatter(df['reference_age_months'].astype(float), df['average_watts_fully_utilized'].astype(float), color='g')
scatter5 = FigureCanvasTkAgg(figure5, root)
scatter5.get_tk_widget().pack(side=tk.RIGHT, fill=tk.BOTH)
ax5.legend()
ax5.set_xlabel('memory')
ax5.set_title('Reference Age Vs. average watts fully utilized')
