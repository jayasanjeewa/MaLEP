...
X = df[['threads_per_core']].astype(float)
...
for row in df_test.itertuples():
    ...
    predicted_idle_power = regr.predict([[test_processor_threads]])

    error_percentage = (abs(predicted_idle_power - idle_power)  / idle_power ) * 100
    total_error_percentage += error_percentage


    print (test_processor_threads,  ",", idle_power, ",", predicted_idle_power, ",", str(error_percentage))
